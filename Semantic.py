...
class Semantic_Reconstruction(nn.Module):
    def __init__(self, h=400, in_dim=13447, out_dim=256*768, seq_len=62, n_blocks=4, drop=.15, clip_size=768, blurry_recon=False, clip_scale=1):
        super().__init__()
    ...
    def forward(self, x):
        ...
        backbone = self.backbone_linear(x).reshape(len(x), -1, self.clip_size) #[b,64,768]
...
class BrainDiffusionPrior(DiffusionPrior):
    ...
    - Option to include the eeg2clip model and pass eegs into forward method
    - Return predictions when computing loss
    - Load pretrained model from @nousr trained on LAION aesthetics
    """
    def __init__(self, *args, **kwargs):
        eeg2clip = kwargs.pop('eeg2clip', None)
        super().__init__(*args, **kwargs)
        self.eeg2clip = eeg2clip
    ...
    def forward(
        ...
        **kwargs
        ...
        assert exists(text) ^ exists(text_embed) ^ exists(eeg), 'either text, text embedding, or eeg must be supplied'
        assert exists(image) ^ exists(image_embed), 'either image or image embedding must be supplied'
        assert not (self.condition_on_text_encodings and (not exists(text_encodings) and not exists(text))), 'text encodings must be present if you specified you wish to condition on it on initialization'

        if exists(eeg):
            assert exists(self.eeg2clip), 'eeg2clip must be trained if you wish to pass in eegs'
            assert not exists(text_embed), 'cannot pass in both text and eegs'
            if self.eeg2clip.use_projector:
                clip_eegs_mse, clip_eegs = self.eeg2clip(eeg)
                text_embed = clip_eegs_mse
            else:
                clip_eegs = self.eeg2clip(eeg)
                text_embed = clip_eegs_mse = clip_eegs
            # text_embed = self.eeg2clip(eeg)
...
if __name__ == "__main__":
    # eeg = np.load('data/SEED-DV/Segmented_Rawf_200Hz_2s/sub1.npy')
    # print(eeg.shape)
    # eeg_tensor = torch.from_numpy(eeg)
    # eeg = eeg_tensor.to(dtype=torch.float32)

    # eeg = rearrange(eeg,'a b c d e  -> a (b c ) d e ')
    # eeg_train = eeg[0]
    # print(eeg_train.shape)

    # model = Semantic_Reconstruction(h=400, in_dim=400, out_dim=256*768, seq_len=62, n_blocks=4, 
    #                                 drop=.15, clip_size=768, blurry_recon=False, clip_scale=1)
    
    # _,y,_ = model(eeg_train)
    pass
